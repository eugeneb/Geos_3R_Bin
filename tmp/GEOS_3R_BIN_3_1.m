...
phData.NumPointPoiskKA=4;    % число точек для поиска нужного КА
...
while(1==1) % обработка данных с заданного КА
    ...
    while(1==1) %обработка сообщения 0х10
        ...
        for(k=1:phData.NumKA)%обработка данных с "к"-го спутника
            [phData.tmp_KAnumber, phData.tmp_reliable, phData.tmp_SignalToNoise,...
                phData.tmp_phase, phData.tmp_Doppler]=GEOS_3R_BIN_DataDecod_0x10(...
                Bin.PH_data(4*(-9+14*k)-3:4*(4+14*k),1)); %считываем информацию с очередного спутника
            if(phData.tmp_KAnumber==ind_KA) % "это данные с к-го спутника?"
                logik=1;
        ...
        if(logik==0) % если на этом шаге нет информации с нужного нам спутника (ind_KA)
